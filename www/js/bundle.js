(function () { function r(e, n, t) { function o(i, f) { if (!n[i]) { if (!e[i]) { var c = "function" == typeof require && require; if (!f && c) return c(i, !0); if (u) return u(i, !0); var a = new Error("Cannot find module '" + i + "'"); throw a.code = "MODULE_NOT_FOUND", a } var p = n[i] = { exports: {} }; e[i][0].call(p.exports, function (r) { var n = e[i][1][r]; return o(n || r) }, p, p.exports, r, e, n, t) } return n[i].exports } for (var u = "function" == typeof require && require, i = 0; i < t.length; i++)o(t[i]); return o } return r })()({
  1: [function (require, module, exports) {
    ...
    function showRadio(time) {
      var inTime = false
      var data = []
      var now = new Date()
      if(!paused)
        document.getElementById('seek').value = now.getTime()
      for (let j = 0; j < observations.length; j++) {
        const o = observations[j];
        if (filter.length > 0)
          if (!filter.includes(o.norad_cat_id))
            continue
        var start = Date.parse(o.start)
        var end = Date.parse(o.end)
        if(!time){
          //Perf
          if (inTime && now.getTime() < start) {
            return {
          ...
          if (now.getTime() > start && now.getTime() < end) {
            inTime = true;
            if (tle[o.norad_cat_id]) {
              var loc = tlejs.getLatLon(tle[o.norad_cat_id])
              var val = {
                "type": "Feature",
                "geometry": {
                  "type": "LineString",
                  "coordinates": [
                    [loc.lng, loc.lat],
                    [o.station_lng, o.station_lat]
                  ]
                },
                "properties": {
                  "description": "<strong>" + o.transmitter_description + "</strong><p style='margin-top: 0;'>" + (o.transmitter_downlink_low / 1000000).toFixed(3) + "MHz</p>"
                }
              }
              data.push(val)
            }
          ...
          if (time > start && time < end) {
            inTime = true;
            if (tle[o.norad_cat_id]) {
              var loc = tlejs.getLatLon(tle[o.norad_cat_id], time)
              var val = {
                "type": "Feature",
                "geometry": {
                  "type": "LineString",
    ...
    function showSats(time) {
      var data = []
      for (let i = 0; i < sats.length; i++) {
        if (filter.length > 0)
          if (!filter.includes(sats[i]))
            continue
        const s = tle[sats[i]]
        if (s) {
          var loc
          if(time)
            loc = tlejs.getLatLon(tle[sats[i]], parseInt(time))
          else
            loc = tlejs.getLatLon(tle[sats[i]])
          data.push({
            "type": "Feature",
            "geometry": {
              "type": "Point",
            ...
            "properties": {
              "description": "<a target='blank' href='https://db.satnogs.org/satellite/" + sats[i] + "'><strong>" + tlejs.getSatelliteName(tle[sats[i]]) + "</strong></a>"
            }
          })
        }
      }
    ...
    map.on('load', function () {
      document.getElementById("playBtn").addEventListener("click", pause); 
      document.getElementById("nowBtn").addEventListener("click", now); 
      document.getElementById("filterBtn").addEventListener("click", filters);
      ...
      document.getElementById('seek').addEventListener('input',function() {
        ...
        var n = new Date(parseInt(this.value))
        var disp = n.getUTCMonth() + "/" + n.getUTCDate() + " " + n.getUTCHours() + ":" + n.getUTCMinutes() + ":" + n.getUTCSeconds()
        document.getElementById("timeTip").textContent = disp
        console.log(this.width)
        var pos = scale(this.value, this.min, this.max, 75, document.getElementById('seek').offsetWidth+55)
        console.log(pos)
        document.getElementById("timeTip").style.left = pos + "px"
        document.getElementById("timeTip").style.display = "block";
      });
      document.getElementById('seek').addEventListener('change',function() {
        document.getElementById("playBtnImg").src="/img/play.png";
        document.getElementById("timeTip").style.display = "none";
      });

      function pause(){
        if(paused){
          document.getElementById("playBtnImg").src="/img/pause.png";
          requestAnimationFrame(animateSats);
        }
        else{
            document.getElementById("playBtnImg").src="/img/play.png";
      ...
      map.on('click', 'satellites', function (e) {
        var coordinates = e.features[0].geometry.coordinates.slice();
        var description = e.features[0].properties.description;
      ...
      // Change the cursor to a pointer when the mouse is over the places layer.
      map.on('mouseenter', 'satellites', function () {
        map.getCanvas().style.cursor = 'pointer';
      });

      // Change it back to a pointer when it leaves.
      map.on('mouseleave', 'satellites', function () {
        map.getCanvas().style.cursor = '';
      });

      map.on('click', 'radio-links', function (e) {
      ...
      // Change the cursor to a pointer when the mouse is over the places layer.
      map.on('mouseenter', 'radio-links', function () {
        map.getCanvas().style.cursor = 'pointer';
      });

      // Change it back to a pointer when it leaves.
      map.on('mouseleave', 'radio-links', function () {
        map.getCanvas().style.cursor = '';
      });
      ...
      function animateSats() {
        ...
        var time
        if(paused)
          time = document.getElementById('seek').value
        map.getSource('sats').setData(showSats(time));
        map.getSource('radios').setData(showRadio(time));
      ...
      //Stations
      getJSON('/api/stations', function (err, data) {
        stations = data
        var mapS = []
        for (let i = 0; i < stations.length; i++) {
          ...
          mapS.push({
            ...
            "properties": {
              "description": "<a target='blank' href='https://network.satnogs.org/stations/" + s.id + "'><strong>" + s.name + "</strong></a>"
            }
          })
        }
      ...
      //TLE
      getJSON('/api/tle', function (err, data) {
        tle = data
      });
      ...
      //Observations
      getJSON('/api/observations?start=' + start.getTime() + '&end=' + end.getTime(), function (err, data) {

        observations = data
        document.getElementById('seek').min = Date.parse(observations[0].start)
        document.getElementById('seek').max = Date.parse(observations[observations.length-1].end)
